...
import { consumerAgentId, consumerTaskId, producerAgentId, producerTaskId, producerWorktreeId, repositoryId, workspaceId, } from "./fixtures.js";
...
function createProxy(store, workspaceRoot, ids, opaque = false) {
    return new McpProxy({
        eventStore: store,
...
function createContext(workspaceRoot, interrupted = false) {
    return {
        source: {
            kind: "adapter",
        ...
        correlationId: `corr_${interrupted ? "d" : "e"}`.padEnd(37, "0"),
        causationId: null,
    ...
    await withTemporaryDirectory("patchmesh-m7-git-", async (root) => {
        ...
        await withTemporaryDatabase(async (databasePath) => {
            ...
            try {
                ...
                const result = await proxy.execute(editCall, { ...createContext(worktree), workspaceRoot: worktree }, async () => {
                ...
                assert.ok(result.observationDiagnostics.some((gap) => gap.kind === "unverified" && gap.scope === "tool.effects"));
    ...
    await withTemporaryDirectory("patchmesh-m7-git-", async (root) => {
        ...
        await withTemporaryDatabase(async (databasePath) => {
            ...
            try {
                ...
                const failed = await failedProxy.execute(editCall, createContext(root), async () => ({ outcome: "failed", error: new Error("password=do-not-persist"), exitCode: 1 }));
    ...
    await withTemporaryDirectory("patchmesh-m7-git-", async (root) => {
        ...
        await withTemporaryDatabase(async (databasePath) => {
            ...
            try {
                ...
                const interrupted = await proxy.execute(editCall, createContext(root, true), async (signal) => {
                    if (signal.aborted)
                        throw new Error("aborted execution");
                    return { outcome: "succeeded", value: true, exitCode: 0 };
                }, controller.signal);
                assert.equal(interrupted.execution.outcome, "interrupted");
                const completion = store.read().at(-1);
                if (completion?.eventType !== "tool.completed")
                    throw new Error("expected interrupted completion");
