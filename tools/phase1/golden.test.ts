...
test("golden observation-only cross-worktree stream projects graph state without findings", async () => {
  await withTemporaryDatabase(async (databasePath) => {
    ...
    try {
      ...
      assert.ok(snapshot.graphSnapshot.nodes.some(
        (node) => node.kind === "resource" && node.resource.locator === "src/contracts.ts#calculateTotal",
      ));
...
test("incremental and clean graph projection snapshots are byte-equivalent", () => {
  ...
  for (const event of events) projector.process(event);

  assert.equal(
    stableDigest(projector.snapshot()),
    stableDigest(projectWorkGraph(events).snapshot),
  );
});
