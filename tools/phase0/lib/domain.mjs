...
export function validateValidityRecords(records) {
  ...
  for (const [index, record] of records.entries()) { const transition = record.lastTransition; if (transition === null) { if (record.validityState !== 'unassessed') diagnostics.push(issue('PHASE0_TRANSITION_INVALID', `/validity/${index}/lastTransition`, 'only unassessed may omit an initial transition')); continue; } const reasons = VALIDITY_TRANSITIONS.get(`${transition.from}->${transition.to}`); if (!reasons?.has(transition.reason) || record.validityState !== transition.to) diagnostics.push(issue('PHASE0_TRANSITION_INVALID', `/validity/${index}/lastTransition`, 'validity transition or guard is invalid')); if (record.validityState !== 'unassessed' && record.executionState !== 'completed') diagnostics.push(issue('PHASE0_TRANSITION_INVALID', `/validity/${index}/executionState`, 'assessed validity requires a completed work product')); const expected = outcome.get(transition.reason); if (expected) { const match = record.validations.find((candidate) => candidate.outcome === expected && candidate.targetSnapshotId === record.targetSnapshotId && transition.evidenceEventIds.includes(candidate.resultEventId)); if (!match || transition.targetSnapshotId !== record.targetSnapshotId) diagnostics.push(issue('PHASE0_TRANSITION_INVALID', `/validity/${index}/validations`, 'transition lacks a matching current-target validation result')); } else if (transition.reason === 'dependency_impact') { if (record.validityState !== 'possibly_stale') diagnostics.push(issue('PHASE0_TRANSITION_INVALID', `/validity/${index}`, 'dependency impact must produce possibly_stale')); } else if (transition.reason === 'target_superseded') { if (!record.validations.some((candidate) => candidate.targetSnapshotId !== record.targetSnapshotId && transition.evidenceEventIds.includes(candidate.resultEventId))) diagnostics.push(issue('PHASE0_TRANSITION_INVALID', `/validity/${index}/validations`, 'superseded transition requires an obsolete-target result')); } }
  return sortDiagnostics(diagnostics);
}
...
export function validateScenarioDomain(scenario) {
  ...
  diagnostics.push(...validateDecisionCapabilities(embeddedDecisions, scenario.manifest.targetPhase), ...validateValidityRecords(embeddedValidity), ...validateDecisionDeliveryEvents(deliveryEvents), ...validateSequenceCoverage(eventSet, scenario.expected?.coverage ?? []));
  ...
  diagnostics.push(...validateDecisionCapabilities(expected.decisions, scenario.manifest.targetPhase), ...validateValidityRecords(expected.validity), ...validateCoverageRecords(expected.coverage)); eventRefs(expected, '/expected', eventById, diagnostics); versionEvidence(expected, '/expected', eventById, diagnostics);
  ...
  for (const edge of graph.edges) { const from = nodes.get(edge.fromNodeId); const to = nodes.get(edge.toNodeId); if (!from || !to) { diagnostics.push(issue('PHASE0_REFERENCE_MISSING', `/graph/edges/${edge.edgeId}`, 'graph edge endpoint is absent')); continue; } const kinds = edgeKinds.get(edge.kind) ?? []; const valid = kinds.some(([a, b]) => from.kind === a && to.kind === b); if (!valid) diagnostics.push(issue('PHASE0_SCHEMA_INVALID', `/graph/edges/${edge.edgeId}`, 'graph edge endpoint kinds are invalid')); if ((edge.kind === 'depends_on') !== (edge.dependency !== null)) diagnostics.push(issue('PHASE0_SCHEMA_INVALID', `/graph/edges/${edge.edgeId}/dependency`, 'only dependency edges may carry dependency provenance')); if (edge.dependency) validateDependency(edge.dependency, `/graph/edges/${edge.edgeId}/dependency`, resourceById, eventById, diagnostics); }
